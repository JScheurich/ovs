...
dpcls_create_subtable(struct dpcls *cls, const struct netdev_flow_key *mask)
{
    ...
    cpvector_insert(&cls->subtables, subtable, 0);
    ...
    OVS_REQUIRES(pmd->flow_mutex)
{
    ...
    struct cpvector *pvec = &cls->subtables;
    CPVECTOR_FOR_EACH (subtable, pvec) {
        cpvector_change_priority(pvec, subtable, subtable->hit_cnt);
        subtable->hit_cnt = 0;
    }
    cpvector_publish(pvec);
}
...
#define MAP_BITS (sizeof(map_type) * CHAR_BIT)
     ...
     * non-overlapping. */
    CPVECTOR_FOR_EACH (subtable, &cls->subtables) {
